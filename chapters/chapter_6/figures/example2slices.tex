...
\begin{tikzpicture}[ %tdplot_main_coords,
	...
	\begin{axis}[
			view={-37}{20}, % first number is how much to rotate around z axis for the view, second is how far above/below you are in angle around x (negative is looking up, positive is looking down)
			axis y line = none,
		...
		\pgfplotsinvokeforeach{1,...,\N+1} {
			\tikzmath{
				...
				\currenty{#1} = \currentx{#1}*\currentx{#1};
			}
			\fill[area plot] (\currentx{#1},0,0) -- (\currentx{#1},-\currenty{#1},0) -- (\currentx{#1},-\currenty{#1},\currenty{#1}) -- (\currentx{#1},0,\currenty{#1}) -- cycle;
			...
			\addplot3 [black,domain=0:4,samples = 100, samples y=0] (x,{-x*x},0);
			\addplot3 [black,domain=0:4,samples = 100, samples y=0] (x,{-x*x},{x*x});
		}
	\end{axis}
\end{tikzpicture}
\end{document}
